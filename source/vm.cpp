...
Vm::Vm(Host* host, PicoRam* memory, Graphics* graphics, Input* input, Audio* audio)
    : _loadedCart(nullptr),
      _luaState(nullptr),
      _cleanupDeps(false),
      _targetFps(30),
...
bool Vm::loadCart(Cart* cart) {
    ...
    // Push the eris.init_persist_all function on the top of the lua stack (or nil if it doesn't
    // exist) we call this function to establish the default global state of things not to save in
    // the save state needs to be called after globals are loaded but before the cart is run, or
    // _init is called
...
void Vm::vm_flip() {
    ...
    if (!_host->shouldQuit() && !_cartChangeQueued) {
        ...
        if (_pauseMenu) {
            // pause menu probably needs refactor out of lua. For now this is better than just
            // quitting
            lua_getglobal(_luaState, "__f08_menu_update");
            lua_call(_luaState, 0, 0);
            lua_pop(_luaState, 0);
